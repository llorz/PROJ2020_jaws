function [C12] = compute_fMap_new(S1,S2,B1,B2,Ev1,Ev2,fct_src,fct_tar,para)
...
myObj = @(C) a*funDesp(C) + b*funCommDesp(C) + c*funCommLB(C) + d*funOrient_direct(C);
myGrad = @(C) a*gradDesp(C) + b*gradCommDesp(C) + c*gradCommLB(C) + d*gradOrient_direct(C);
