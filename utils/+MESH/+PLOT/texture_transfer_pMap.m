...
%ic = [1,2,3];
S2_vt = MESH_IO.generate_tex_coords(X2(:,ic), cols(2), cols(1), 1);

S2_vt(:,1) = 2*(S2_vt(:,1) - min(S2_vt(:,1)))/(max(S2_vt(:,1)) - min(S2_vt(:,1)));
S2_vt(:,2) = 2*(S2_vt(:,2) - min(S2_vt(:,2)))/(max(S2_vt(:,2)) - min(S2_vt(:,2)));
